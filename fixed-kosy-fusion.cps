...
var settings = {
    retract: {
        ...
        useZeroValues: ["G30 Z30"], // enter property value id(s) for using "0" value instead of machineConfiguration axes home position values (ie G30 Z0)
...
function onOpen() {
    ...
    // required header
    //UPDATE removed - not recognised by KOSY
    //writeBlock("_");
    //writeBlock("_");
    ...
    // UPDATE G76 Command broken due to broken limit sensor
...
function onCommand(command) {
    switch (command) {
        ...
        case COMMAND_LOAD_TOOL:
            // CHANGED - do not load tool at start of every program
...
function writeInitialPositioning(position, isRequired, codes1, codes2) {
    ...
    writeStartBlocks(isRequired, function () {
        ...
        // UPDATE Change - this addition of G90 forces a write of G90 and G0 Xx Yy Zy on the same line
        // removing the G90 prevents that from happening 
        // var modalCodes = formatWords(gAbsIncModal.format(90), gPlaneModal.format(17));
        var modalCodes = formatWords(gPlaneModal.format(17));
